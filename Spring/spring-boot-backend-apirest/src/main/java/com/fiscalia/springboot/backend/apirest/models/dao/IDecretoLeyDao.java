...
public interface IDecretoLeyDao  extends JpaRepository<DecretoLey, Long> {
	...
	@Query("select p from DecretoLey p where p.titulo like %?1%")
	...
	@Query("select p from DecretoLey p where p.numero like ?1")
	public List<DecretoLey> findByNumero(String term);
	
	@Query("select p from DecretoLey p where p.anio like ?1")
	public List<DecretoLey> findByAnio(String term);
